...
- Where enforced: `finalize-plan/route.ts` (one call, a comment explains
...
**AI call caps** — `Event.aiCallsUsed`, checked as
`AI_CALL_LIMIT` locally in each route (NOT centralized). Caps are per-route and
drift (10 vs 20 as of 2026-07-09; the god-file UI copy
...
- V1 path: `PLAN_GENERATION_SYSTEM_PROMPT` + `buildGenerationPrompt`
  + regeneration variants — NZ rules HARDCODED in the system prompt text. Used by `/api/events/[id]/generate` and `/regenerate` via
  `src/lib/ai/generate.ts`.
- V2 path: `buildPlanGenerationPrompt` — NZ rules injected via
...
1. **Path-scoped token cookies** — for page routes `/p/`, `/h/`, `/c/`, the URL
   ...
   each scoped to its own path (`TOKEN_PAGE_COOKIES`; 8-hour maxAge).
...
2. **Session-cookie stripping** — for `/api/p/` and `/api/c/` requests, the
   ...
   (`SESSION_STRIP_PREFIXES`). Those handlers authenticate purely via
   `resolveToken()` (`src/lib/auth.ts` — `resolveToken()`).
   **`/api/h/` is INTENTIONALLY excluded** — the host token view calls
   `getUser()` (`src/lib/auth/session.ts` — `getUser()`) and needs the session cookie present.
...
**Where enforced:** `middleware.ts` (the matcher limits it to the
...
|---|---|---|---|---|
| Event membership | `PersonEvent.role` | `PersonRole`: HOST, COORDINATOR, PARTICIPANT | "What is this person's job within this event's plan?" | Plan logic, team membership, nudges |
| Login-account permission | `EventRole.role` (`prisma/schema.prisma` — `EventRole.role`) | `EventRoleType`: HOST, COHOST, COORDINATOR | "What can this signed-in USER account do to this event?" | Session-auth routes via `requireEventRole` (`src/lib/auth/guards.ts` — `requireEventRole()`), `canEditEvent` (`src/lib/entitlements.ts`) |
| Link capability | `AccessToken.scope` (`prisma/schema.prisma` — `AccessToken.scope`) | `TokenScope`: HOST, COORDINATOR, PARTICIPANT | "What may the holder of this URL do?" | Token routes via `requireTokenScope` (`guards.ts` — `requireTokenScope()`), `resolveToken` |
...
**Transitions** (`canTransition`, `workflow.ts` — `canTransition()`):
...
**Mutation gating** (`canMutate`, `workflow.ts` — `canMutate()`; actions: createItem, editItem,
...
- **DESIGN DECISION (2026-07-09): CONFIRMING→FROZEN never hard-blocks on
  ...
  (`src/app/api/events/[id]/transition/route.ts` — the `CONFIRMING` branch,
  `if (event.status === 'CONFIRMING')`) calls
  `checkFreezeReadiness` (`workflow.ts` — `checkFreezeReadiness()`), whose result type sets
...
- `computeTeamStatusFromItems`: "Does NOT use cached Item.status".
- `checkFreezeReadiness`: the live freeze check — its
...
- `repairItemStatusAfterMutation`: the canonical cache-repair helper —
...
`src/app/api/events/[id]/items/[itemId]/assign/route.ts`) instead
...
**Where enforced:** `src/lib/workflow.ts` transactions in
`removePerson`, `transitionToConfirming`, `createRevision`,
`restoreFromRevision`; ~16 API routes use `$transaction` (grep below).
`logAudit` (`workflow.ts` — `logAudit()`) REQUIRES a `tx` — "All audit logging must happen
inside transactions."
...
checkout session. `POST /api/events` (`src/app/api/events/route.ts`)
...
(`paidAt`, `amountPaid` — schema).

**Vestigial but present:** the `Subscription` model,
...
2. **Household edit = delete-and-recreate** —
   ...
   non-primary `PersonEvent` rows (the `personEvent.deleteMany` in the PUT handler) and recreates members. Consequences:
   ...
   matches on email only). Also runs with NO transaction. This was a

...
| 13 | Plan header says "based on 0 guests" (or wrong headcount) | Reading `Event.guestCount` (nullable, host-entered, often null) instead of the Household aggregate | The display source is `planGuestCount` in `src/app/plan/[eventId]/page.tsx`: households headcount, falling back to `event.guestCount ?? 0` | One canonical headcount = aggregate over Household members (+ `littleCount`). Any NEW headcount display must use it, not `Event.guestCount` | GTC-136 (5154252) |
...
label)` in `src/lib/ai/claude.ts` now logs

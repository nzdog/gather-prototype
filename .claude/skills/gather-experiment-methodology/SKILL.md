...
|---|---|
| **Temperature variance** — AI output varies run to run. Verified current settings (as of 2026-07-09): default `temperature = 1.0` in `src/lib/ai/claude.ts` — `callClaude()` (the `config.temperature ?? 1.0` fallback); V2 finalize-plan passes `0.8` (`src/app/api/events/[id]/finalize-plan/route.ts` — the `finalize-plan:full` `callClaudeForJSON` call in `POST()`); legacy V1 `src/lib/ai/generate.ts` passes `1.0` | Run the generation MULTIPLE times before trusting a delta. GTC-146 treated a 3-item difference (25 vs 28) as within variance. A single lucky run proves nothing — if your measured improvement is smaller than run-to-run spread, you have measured noise |

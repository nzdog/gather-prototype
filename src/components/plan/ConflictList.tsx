        ...
        <div className="bg-sage-50 border border-sage-200 rounded-lg p-8 max-w-md mx-auto">
          <div className="text-center">
            ...
            <p className="text-sm text-sage-600 mb-4">
              Check your plan to assess for potential conflicts
            </p>
            {onCheckPlan && (
              <button
                onClick={onCheckPlan}
                disabled={isCheckingPlan}
                className={`px-4 py-2 bg-accent text-white text-sm font-medium rounded-md hover:bg-accent-dark transition-colors inline-flex items-center gap-1.5 ${isCheckingPlan ? 'opacity-75 cursor-not-allowed' : ''}`}
              >
                {isCheckingPlan ? (
                  <>

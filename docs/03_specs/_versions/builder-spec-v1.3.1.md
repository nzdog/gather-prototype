# Gather: Builder's Specification — v1.3.1 (Patch)

## For Claude Code Implementation

**Version:** 1.3.1 (Patch from v1.3)  
**Date:** December 2025  
...
1. **repairItemStatus() made async** — Function signature fixed; call site ordering specified (repair all items → then computeTeamStatus)
2. **Timezone-safe date construction** — Added `makeNzDate()` helper; storage strategy documented (UTC internally, NZ local for display)
3. **Coordinator route scoping invariant** — APIs must scope to `token.teamId` only; never accept teamId from client
4. **Repair-on-read operationalized** — Exact call sites specified: GET routes before returning items, and before team status computation
5. **Team deletion safety** — Added `onDelete: Restrict` to PersonEvent.team; added team deletion to "What NOT to build"
...
- **[ADD: [x] repairItemStatus() is async and call ordering is specified]**
- **[ADD: [x] All seed dates use timezone-safe construction (NZ local → UTC)]**
- **[ADD: [x] Coordinator APIs scoped to token.teamId only]**
...
- **[ADD: ...]** = new content
- For replacements: strike old, then add new
...
model PersonEvent {
  ...
  **[ADD: team     Team   @relation(fields: [teamId], references: [id], onDelete: Restrict)]**
...
| **[ADD: Team cannot be deleted if members exist]** | **[ADD: `onDelete: Restrict` on PersonEvent.team]** |
...
**[ADD: 7. Coordinator API scoping:** All coordinator API routes MUST:
...
**[ADD: ### Coordinator Route Scoping Rules
...
~~If during read a mismatch is detected between `Item.status` and Assignment existence:~~

~~```typescript~~
~~function repairItemStatus(item: Item & { assignment: Assignment | null }): ItemStatus {~~
...
~~This ensures Item.status is always consistent with reality.~~
...
async function getRepairedTeamStatus(
  ...
  // Step 1: Repair all items first (sequential to avoid race conditions)
 ...
 * Use this for all seed data to ensure timezone correctness.
 * 
 ...
 * 
 * December 2025 is NZDT (New Zealand Daylight Time = UTC+13)
 */
function makeNzDate(dateStr: string, timeStr: string): Date {
  const [year, month, day] = dateStr.split('-').map(Number);
  const [hour, minute] = timeStr.split(':').map(Number);
  
  // NZDT offset: UTC+13
  const nzOffsetMinutes = 13 * 60;
  
  // Create a date in UTC that represents this NZ local time
  // NZ local 12:00 = UTC 23:00 previous day (12:00 - 13:00 = -01:00 → 23:00 prev day)
  const utc = Date.UTC(year, month - 1, day, hour, minute, 0, 0);
  const adjusted = new Date(utc - nzOffsetMinutes * 60 * 1000);
  
  return adjusted;
}

// Examples (verify these in implementation):
// makeNzDate("2025-12-24", "17:30") → 2025-12-24T04:30:00.000Z (5:30pm NZ)
// makeNzDate("2025-12-25", "12:00") → 2025-12-24T23:00:00.000Z (12 noon NZ)
// makeNzDate("2025-12-25", "10:00") → 2025-12-24T21:00:00.000Z (10am NZ)

export { makeNzDate };
```

**Display formatting:** When rendering `dropOffAt` in UI, convert UTC → NZ local using `Intl.DateTimeFormat` with `timeZone: 'Pacific/Auckland'`.**]**
...
const event = {
  ...
  **[ADD: startDate: makeNzDate("2025-12-24", "00:00"),]**
  **[ADD: endDate: makeNzDate("2025-12-26", "23:59"),]**
  status: "CONFIRMING",
...
const days = [
  ...
  **[ADD: { name: "Christmas Eve", date: makeNzDate("2025-12-24", "00:00") },]**
  **[ADD: { name: "Christmas Day", date: makeNzDate("2025-12-25", "00:00") },]**
  **[ADD: { name: "Boxing Day", date: makeNzDate("2025-12-26", "00:00") },]**
];
```
...
const dropOff = {
  eve:   { at: makeNzDate("2025-12-24", "17:30"), location: "Kate's Kitchen", note: "5:30pm" },
  day:   { at: makeNzDate("2025-12-25", "12:00"), location: "Marquee", note: "12 noon" },
  box:   { at: makeNzDate("2025-12-26", "12:00"), location: "Marquee", note: "12 noon" },
  setup: { at: makeNzDate("2025-12-25", "10:00"), location: "Marquee", note: "10:00am" },
};
```**]**
...
**[ADD: Note: All items use `dropOff.eve.at`, `dropOff.day.at`, `dropOff.box.at`, or `dropOff.setup.at` for timezone-safe dropOffAt values.]**
...
if (item.assigneeName) {
  ...
  // Verify team match before creating Assignment
  if (personEvent.teamId === teamId) {
    await prisma.assignment.create({ ... });
    await prisma.item.update({ where: { id: item.id }, data: { status: 'ASSIGNED' } });
  } else {
    throw new Error(`Cannot assign ${item.assigneeName} to item in ${item.teamName} — person is in different team`);
  }
}
```
...
6. Create `prisma/seed.ts`:
   - **[ADD: Import `makeNzDate` from lib/timezone.ts]**
   ...
   - Validate Assignment person is in same team as item
...
**[ADD: 9. Create `lib/timezone.ts`:
   - Export `makeNzDate()` helper]**

### Phase 4: Participant View (Day 2)

10. Create `/p/[token]/page.tsx`
11. Create `GET /api/p/[token]` **[ADD: — call `repairItemStatus()` on each item before returning]**
12. Create `POST /api/p/[token]/ack/[assignmentId]`

### Phase 5: Coordinator View (Day 2)

13. Create `/c/[token]/page.tsx`
14. Create `GET /api/c/[token]` **[ADD: — call `getRepairedTeamStatus(items)` before returning; scope queries to token.teamId only]**
15. Create `POST /api/c/[token]/items` **[ADD: — force teamId from token, never from client]**
16. Create `PATCH /api/c/[token]/items/[itemId]` **[ADD: — verify item.teamId === token.teamId before mutation]**
...
19. Create `GET /api/h/[token]` **[ADD: — call `getRepairedTeamStatus(items)` for each team before returning]**
...
21. Create `lib/workflow.ts`:
    ...
    - **[ADD: `repairItemStatus(item)` — async, mutates item.status in DB and in-place]**
    - **[ADD: `getRepairedTeamStatus(items)` — repairs all items, then calls computeTeamStatus]**
...
**[ADD: 28. Test coordinator route scoping (cannot access other teams' items)]**
**[ADD: 29. Test Item.status repair (verify mismatch is corrected on read)]**
...
- **[ADD: [ ] All queries scoped to token.teamId only]**
- **[ADD: [ ] Item creation uses teamId from token, never from client]**
- **[ADD: [ ] Cannot access items where item.teamId !== token.teamId]**
...
- **[ADD: [ ] `repairItemStatus()` called before returning items from GET routes]**
- **[ADD: [ ] `getRepairedTeamStatus()` called before computing team status]**
- [ ] 55 items loaded
...
- **[ADD: [ ] Team deletion (blocked by `onDelete: Restrict` on PersonEvent.team)]**
- [ ] Offline support
- [ ] Mobile app
...
│   │   ├── workflow.ts             # Mutation guards, computeTeamStatus, repairItemStatus, getRepairedTeamStatus
│   │   **[ADD: └── timezone.ts             # makeNzDate() helper]**
│   └── components/
│       ├── ItemCard.tsx
│       ├── TeamCard.tsx
│       ├── StatusBadge.tsx
...
```
Read the attached Gather Builder's Specification v1.3.1 completely.
...
- Call repairItemStatus() before returning items from GET routes
...
- Use makeNzDate() for all DateTime construction in seed data (Pacific/Auckland timezone)
...
Attached: gather-builder-spec-v1.3.1.md
```

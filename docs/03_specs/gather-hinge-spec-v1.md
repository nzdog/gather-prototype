...
- **Owner and channel are separate fields.** Grandma has no phone: her ask routes to the person Kate always reaches her through — but the pavlova remains *Grandma's*, her name on the yellow box and on the fridge. The system never needs to know she has no phone; it only needs to know who Kate said to talk to.
...
- **The colours know the difference.** Never-reached (red at once) can never be mistaken for reached-and-quiet (yellow, with its nudge-clock). No silence gets nudged into a void for days and goes red for the wrong reason.
...
**Colour: yellow.** The system can work a maybe — just not with the silence machinery.
...
**One line for the build:** a maybe is a surfaced decision, yellow, on a derived-with-override decide-by clock instead of a nudge cadence; red through the standard door if the clock expires; the item held softly — Kate can reassign with a why, and the system notifies the released guest.

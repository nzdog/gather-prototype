# Gather: Builder's Specification — v1.3.2
...
The coordinator GET route returns status indicators for other teams WITHOUT exposing their items. This MUST be implemented as a server-side aggregate computation:
...
// 2. Compute other teams' statuses via aggregate query
// NEVER fetch other teams' items into coordinator context
const otherTeamsStatus = await prisma.team.findMany({
  where: { 
...
// 3. Map to status enum
const otherTeams = otherTeamsStatus.map(t => ({
  id: t.id,
  name: t.name,
  status: t._count.items > 0 ? 'CRITICAL_GAP' : 'SORTED' // Simplified for prototype
}));

// 4. Return coordinator's items + other teams' statuses only
return Response.json({
  team: { id: tokenTeam.id, name: tokenTeam.name },
  items: myItems,
  myStatus,
  otherTeams, // Status only, no items
});
```

**Critical constraint:** The `otherTeams` array must NEVER contain item data. Coordinator routes must not fetch items where `teamId !== token.teamId` except via aggregate count queries.
...
14. Create `GET /api/c/[token]` — call `getRepairedTeamStatus(items)` before returning; scope queries to token.teamId only; compute other teams' statuses via aggregate query
...
- [ ] Coordinator GET returns other teams' statuses without exposing their items
...
│   │   ├── workflow.ts             # Mutation guards, computeTeamStatus, repairItemStatus, getRepairedTeamStatus, canFreeze, getCriticalGapCount
...
```
Read the attached Gather Builder's Specification v1.3.2 completely.
...
- COORDINATOR routes compute other teams' statuses server-side via aggregate query; never fetch other teams' items
...
Attached: gather-builder-spec-v1.3.2.md
```
...
|---|------|--------|------------|--------|
| 1 | Other teams' status aggregation may miss GAP (non-critical unassigned) | Coordinator sees simplified status (CRITICAL_GAP or SORTED) instead of full GAP distinction for other teams | Low impact for prototype; full status would require additional aggregate query. Document as known limitation. | Accepted |

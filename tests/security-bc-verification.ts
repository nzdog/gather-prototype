...
const MUTATION_ROUTES = [
  ...
  { path: '/api/events', method: 'POST', auth: 'SESSION', body: {name:'T',startDate:'2026-01-01',endDate:'2026-01-02'} },
  { path: (id:string)=>`/api/events/${id}/teams`, method: 'POST', auth: 'SESSION', body: {name:'T'} },
  { path: (id:string)=>`/api/events/${id}/people`, method: 'POST', auth: 'SESSION', body: {name:'T'} },
  
  // Coordinator token-based
  { path: '/api/c/TOKEN/items', method: 'POST', auth: 'COORDINATOR', body: {name:'T',teamId:'TEAM'} },
];
...
async function testWrongTeamFails() {
  ...
  const res = await fetch(`${BASE_URL}/api/c/${fixtures.teamA.coordinator.token}/items/${fixtures.teamB.items[0].id}`, {
    method: 'PATCH',
    headers: {'Content-Type':'application/json'},
